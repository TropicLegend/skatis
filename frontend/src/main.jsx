...
function ListWorkspace({ list, tournament, role, token, onBack, onLogout, onUpdated }) {
  ...
  return <Shell tournament={tournament} role={role} onLogout={onLogout} token={token} eyebrow="Tischliste">
    ...
    {showWizard && <GameWizard list={list} existingGame={editingGame} onClose={() => { setShowWizard(false); setEditingGame(null) }} onSave={saveGame} />}
    {detailGame && <GameDetail list={list} round={detailRound} onClose={() => setDetailGame(null)} onEdit={role === 'ADMIN' ? () => { setDetailGame(null); setEditingGame(detailGame); setShowWizard(true) } : null} />}
  </Shell>
}
 ...
 * gibt in Runde 1.
 */
function LineupPicker({ players = [], value = [], onChange }) {
  const toggle = (name) => {
  ...
  return <fieldset className="lineup-picker">
    ...
    <p>{players.length ? 'Antippen wählt aus – die Reihenfolge der Auswahl ist die Sitzreihenfolge.' : 'Noch keine Spieler im Roster. Lege zuerst unten im Roster Namen an.'}</p>
    <div className="lineup-choices">{players.map((player) => {
      const seat = value.indexOf(player.name)
      return <button type="button" key={player.name} className={`lineup-choice ${seat >= 0 ? 'selected' : ''}`} disabled={seat < 0 && value.length >= 5} onClick={() => toggle(player.name)}>{seat >= 0 ? <b>{seat + 1}</b> : <Plus size={13} />}{player.name}</button>
    })}</div>
    <div className="lineup-status"><span className={value.length >= 3 ? 'ok' : ''}>{value.length} von 3 bis 5 gewählt</span>{value.length > 0 && <button type="button" className="text-button" onClick={() => onChange([])}>Auswahl leeren</button>}</div>
  </fieldset>
}
...
function GameDetail({ list, round, onClose, onEdit }) {
  if (!round) return null
  const { game, delta, before, after } = round
  const lineup = (list.players || []).map((player) => player.name)
...
function GameWizard({ list, existingGame, onClose, onSave }) {
  ...
  const roundNumber = existingGame ? existingGame.position : (list.gameCount || list.games?.length || 0) + 1
    ...
    <div className="wizard">
      ...
      <div className="wizard-content">
        ...
        {step === 1 && <div className="wizard-section">
          <div className="choice-grid players-choice">{players.map((player) => <button key={player} className={`choice-card player-card ${game.declarer === player ? 'selected' : ''}`} onClick={() => setGame({ ...game, declarer: player, passedOut: false })}><span className="avatar">{player[0]}</span><span>{player}</span>{game.declarer === player && <Check size={17} />}</button>)}</div>
        ...
        {step === 2 && <div className="wizard-section">
          ...
          {game.gameType && <div className="options-block">{game.gameType === 'NULL' ? <NullVariantOptions variant={game.nullVariant} onChange={(option) => setGame({ ...game, ...option })} /> : <>
        ...
        {step === 3 && <div className="wizard-section">
          ...
          <div className="number-picker">
            ...
            {maxMatadors(game.gameType) === 11 && <button className={game.matadors.count > 4 ? 'selected wide' : 'wide'} onClick={() => setCustom(true)}>Mehr …</button>}
          </div>
          <div className="tip-box"><span className="tip-number">i</span><p>{maxMatadors(game.gameType) === 4 ? 'Bei Grand sind maximal 4 Spitzen möglich.' : 'Bei Farbspielen kannst du bis 11 Spitzen wählen.'}</p></div>
          {custom && <div className="mini-modal"><label>Exakte Anzahl Spitzen<input type="number" min="1" max="11" value={game.matadors.count} onChange={(e) => setGame({ ...game, matadors: { ...game.matadors, count: Number(e.target.value) } })} /></label><button className="primary-button" onClick={() => setCustom(false)}>Übernehmen</button></div>}
        </div>}
